#!/usr/bin/env node
/**
 * Candystorm 专用 MCP（stdio）— IR/meta + genbot bind 生成，避免通用 Cocos MCP 撑爆上下文。
 *
 * Cursor mcp.json 示例见 README.md
 */
...
const DEFAULT_IR_ROOT = "D:/svn/new_game/糖果风暴客户端资源/export/candystorm_ir";
const CANDYSTORM_ROOT = path.join(PROJECT_ROOT, "assets/asset_bundles/game_art/ab/candystorm");
const CREATOR_BRIDGE = process.env.CANDYSTORM_IR_HTTP_URL || "http://127.0.0.1:3921";
...
const EXPECTED_META = [
    ...
    "assets/asset_bundles/game_art/ab/candystorm/ui/rasters/candystorm_kuang.png.meta",
];
...
function metaStatus() {
    ...
    const manifestPath = path.join(CANDYSTORM_ROOT, "SYNC_MANIFEST.json");
    ...
    return {
        ...
        missing,
        candystormRoot: CANDYSTORM_ROOT,
        irRoot: process.env.CANDYSTORM_IR_ROOT || DEFAULT_IR_ROOT,
        syncManifest,
    };
}
...
server.tool(
    ...
    async ({ only, dryRun }) => {
        ...
        const r = await runNodeScript("scripts/sync-candystorm-ir.mjs", args, {
            CANDYSTORM_IR_ROOT: process.env.CANDYSTORM_IR_ROOT || DEFAULT_IR_ROOT,
        });
        return {
            content: [
                {
...
server.tool(
    "candystorm_refresh_meta_in_editor",
    "Creator 已打开且 fg-candystorm-ir 扩展已启用时，通过本地 HTTP 刷新 asset-db 生成 meta",
    {},
    async () => {
        try {
            const health = await fetchCreatorBridge("/health");
                            ...
                            text: JSON.stringify(
                                {
                                    ...
                                    url: CREATOR_BRIDGE,
                                    hint: "Open Cocos Creator with this project and enable extension fg-candystorm-ir",
                                    health,
                                },
                                null,
                                2,
                        ...
                        text: JSON.stringify(
                            {
                                error: String(e),
                                url: CREATOR_BRIDGE,
                                hint: "Start Creator + fg-candystorm-ir, or use candystorm_import_meta",
                            },
                            null,
                            2,
                        ),
...
server.tool(
    ...
    ].join(" "),
    {
        ...
        preferEditor: z.boolean().optional().describe("优先 Creator HTTP（需 fg-candystorm-ir + genbot 扩展）"),
    },
...
server.tool(
    "candystorm_exec",
    [
        "在已打开的 Cocos Creator 中执行（需 fg-candystorm-ir，不走子进程、不构建）。",
    ...
    ].join(" "),
    {
        ...
        name: z
            .string()
            .optional()
            .describe("scene-script：扩展包名，默认 fg-candystorm-ir；也可调其他扩展 scene 脚本"),
                            ...
                            text: JSON.stringify(
                                {
                                    ...
                                    error: "Creator bridge not reachable",
                                    url: CREATOR_BRIDGE,
                                    hint: "Open Creator, enable fg-candystorm-ir, reload extension if needed",
                                    health,
                                },
                                null,
                                2,
...
async function main() {
    ...
    console.error("[candystorm-mcp] running on stdio (8 tools)");
}

main().catch((e) => {
    console.error("[candystorm-mcp] fatal", e);
    process.exit(1);
});
